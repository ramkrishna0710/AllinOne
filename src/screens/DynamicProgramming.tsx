...
type DynamicProgrammingNavigationProp = StackNavigationProp<RootStackParamList, 'DynamicProgramming'>;

type Props = {
  navigation: DynamicProgrammingNavigationProp;
};

const DynamicProgramming: React.FC<Props> = ({ navigation }) => {
  ...
  return (
    <View style={[{ flex: 1, backgroundColor: theme.colors.primary }, horizontal ? null : { paddingVertical: 30 }]}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 4 }}>
        <Text style={[{ fontWeight: 'bold', margin: 12, color: theme.colors.white }, horizontal ? { fontSize: 16 } : { fontSize: 20, alignItems: 'center' }]}>Dynamic Programming</Text>
        <>
          {horizontal ?
            <TouchableOpacity onPress={() => {
              navigation.navigate('DynamicProgramming', { horizontal: false, })
            }}>
